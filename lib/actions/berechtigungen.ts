...
export async function legeNutzerAn(input: NeuerNutzerInput): Promise<NutzerErgebnis> {
  ...
  const { data: angelegt, error: createError } = input.passwort
    ? await admin.auth.admin.createUser({ email, password: input.passwort, email_confirm: true })
  ...
  const aufraeumen = async () => {
    ...
    await admin.auth.admin.deleteUser(angelegt.user.id);
  };
...
export async function setzeNutzerPasswort(userId: string, passwort: string): Promise<NutzerErgebnis> {
  ...
  if (fehler) return { ok: false, error: fehler };
  const { error } = await pruefung.admin.auth.admin.updateUserById(userId, { password: passwort });
...
export async function loescheNutzer(userId: string): Promise<NutzerErgebnis> {
  ...
  const { error } = await pruefung.admin.auth.admin.deleteUser(userId);
